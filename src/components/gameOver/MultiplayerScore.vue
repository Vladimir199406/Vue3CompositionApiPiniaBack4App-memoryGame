<template>
  <div class="flex flex-col gap-6 sm:gap-10">
    <div class="text-center">
      ...
      <p class="text-title font-bold text-sm sm:text-lg">{{ staticText.gameOver }}</p>
    </div>
    <div class="flex flex-col gap-2 sm:gap-4">
      <div
        v-for="({ player, score }, index) in playerScores"
        :key="index"
        class="rounded-lg font-bold px-4 py-3 flex justify-between items-center sm:px-8"
        :class="
          score === highestScore
            ? ['bg-body-bg', 'text-white']
            : [
                'bg-secondary-idle',
                '[&>p:first-child]:text-title',
                '[&>p:last-child]:text-dark-active'
              ]
        "
      >
        <p class="text-xs sm:text-lg">
          Player {{ player }} <span v-if="score === highestScore">(Winner!)</span>
        </p>
        <p class="text-xl sm:text-3xl">{{ score }} Pairs</p>
      </div>
    </div>
  </div>
</template>
